...
| --- | --- | --- | --- |
| G01.4a — fast feedback orchestration | T; upstream-runner/orchestration owner, coordinate other G01 edits | Add debug-worker mode and hash-validated suite caching without weakening assertions; manifest-selected tests, debounced single-process validation, zero-selection/cache-tamper/stale-source negative tests; compare selected results with the release runner. Feedback timing is not an acceptance benchmark. Validation manifest (2026-09-16): own `scripts/run_upstream.py`, `scripts/measure_upstream_feedback.py`, their focused tests, `benchmark/g01_4a_feedback_workloads.json`, and this row; fast checks: `PYTHONPATH=scripts python3 -m unittest scripts/test_run_upstream.py scripts/test_measure_upstream_feedback.py`. Selected fixture `test/sql/cte/cte_schema.test` is source-mapped and hash-checked in both pins; debug mode compares normalized outcomes to release. Cache records target identity plus every extracted file hash and is rejected on tamper; watcher locking/debouncing is single-process and marks source changes stale. Negatives: empty/unknown selection, cache tamper, stale source, and deliberately wrong result all fail. Functional acceptance: selected debug+release pair with `--compare-release`, followed by the ordinary release runner on the same list. Performance: `measure_upstream_feedback.py` performs three warmups and 21 serial samples against both source-mapped pinned C++ unittest binaries; every Rust sample uses a fresh cache and includes materialization, worker launch, unchanged assertions and report emission. It retains wall time, throughput, CPU, peak RSS and block I/O per sample under `target/`, rejects wrong pins/binaries/results, and gates every metric against the faster pin. Status: **open** until its quiet-host command passes. | [P: at-parity or better performance](#at-parity-or-better-performance) |
