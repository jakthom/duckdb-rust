...
| B2 / G10.2 | Scalar-macro source implementation now carries capture/qualification/default/named arguments and Rust-native checkpoint/WAL create/replace/drop/reopen metadata; validation and two-pin exchange/performance evidence remain open. Table macros remain a separate leaf after F1. Needs B1 catalog lifecycle. | S | New macro cases plus the `scalar_overload` filter in `contracts` and the `sql` target; expansion recursion, shadowing, lazy errors, prepared reuse, rollback and native reopen. | Scalar expansion and repeated parameterized table macro queries, DDL and durable reopen. |
