...
`target/batch3/e1-acceptance-prepare2/`. Independent resources were measured in `target/batch3/e1-resource-acceptance8/`:
all nine cases have valid 21/3 samples and identities, but the strict gate fails.
The eight sort cases use roughly twice reference peak RSS despite wall ratios
0.490–0.502 and CPU ratio 0.469; reject/reuse fails wall 1.146 and CPU 1.25.
The corrected immutable `target/batch3/e1-reject-fixture7/inputs7` preserves
all query populations and ordered oracles, places fixtures beneath discoverable
