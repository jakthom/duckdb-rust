...
54.203667 ms Rust wall median versus 40.623458 ms for the faster C++ pin, or
1.334295x; invocation throughput is 0.749x and fails by the same wall-time
comparison. Rust CPU is 0.667x and RSS 0.422x the faster C++ medians, and block
...
- **Repaired G03.3a engine mismatch:** `enum_range_boundary(k, NULL)` now preserves
  ...
  runtime vector encoding: an executed constant NULL suppresses later arguments,
  including through an all-selected CASE/NULLIF branch and overload cast, while a
  mixed or flat NULL does not.
...
`target/g01-wave-c-20260915/performance-final-6/`). All reports retain the same
...
1.123x, CPU 1.143x, RSS 1.250x, throughput 0.890x); G04.2a
`make_date(STRUCT)` is **fail** (wall 10.184x, CPU 29.000x, RSS 1.861x,
throughput 0.098x); G06.1a case conversion is **fail** (wall 2.916x, CPU
4.000x, RSS 0.801x, throughput 0.343x); G07.3a DISTINCT ON is **fail** (wall
2.514x, CPU 3.000x, RSS 2.056x, throughput 0.398x); and G08.1a `product` is
**fail** (wall 18.117x, CPU 25.333x, RSS 0.282x, throughput 0.055x). G10.4a
profiling with `no_output` is a measured **pass** (wall 0.634x, CPU 0.000x at
timer resolution, RSS 0.751x, throughput 1.578x), but its overall Gate P remains
...
1.123x, CPU 1.143x, RSS 1.250x and throughput 0.890x the faster pin).
...
**at-parity or better performance** gate fails (wall 10.184x, CPU 29.000x and RSS
1.861x the faster pin).
...
performance** gate fails (wall 2.916x and CPU 4.000x the faster pin, despite
0.801x RSS).
...
2.514x, CPU 3.000x, RSS 2.056x and throughput 0.398x the faster pin). This is
...
18.117x, CPU 25.333x and throughput 0.055x the faster pin, despite 0.282x RSS).
