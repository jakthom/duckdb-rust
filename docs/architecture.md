...
| SQLLogic parity harness | [integrated runner](../test/runner/mod.rs), [loop scheduler](../test/runner/schedule.rs), [session lifecycle](../test/runner/session.rs), [byte parser/accounting](../test/runner/parser.rs), [fixtures/directives](../test/runner/fixtures.rs), [typed oracle](../test/runner/oracle.rs), [upstream proxy](../scripts/sqllogic.py) | Source-rooted loops/foreach, named sessions, concurrent join/stop behavior and load/restart/reconnect now exist; test-config max-thread routing, load-version selection and proxy result-mismatch short-circuiting remain explicit G01.2c limits |
