...
| Table sources | [table-function registry and integer range](../src/function/table/mod.rs), [scan lifecycle guard](../src/storage/table_function.rs) | F1 integrated at `449d92c`, acceptance in progress: immutable binding, per-open state and cleanup; temporal/correlated sources and general external formats remain open |
