...
| Table sources | [table-function registry and integer range](../src/function/table/mod.rs), [scan lifecycle guard](../src/storage/table_function.rs) | F1's lifecycle/integer-range slice is accepted through `324878c` with final performance at `90d63ab`: immutable binding, per-open state and cleanup; temporal/correlated sources and general external formats remain open |
