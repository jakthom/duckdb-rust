...
def merge_population(initial, retry, target):
    ...
    if len(selected) != len(base["selected"]) or set(original) != selected:
    ...
    if retry_paths != timeouts or set(retry_results) != retry_paths or len(retry_results) != len(rerun["results"]):
        raise ValueError(f"{target}: retry ids are not exactly initial timeout ids")
    effective = [retry_results.get(r["path"], r) for r in base["results"]]
    ...
    for r in executable:
        if r.get("source_sql_records") == 0:
            r = r  # Report normalization only; raw reports remain unchanged.
            r["status"] = "incomplete"
            r["failure_class"] = "controls_only" if r.get("attempted_records", 0) else "no_sql_records"
...
def main():
    ...
    result = {"scope": "Normalized G01 SQL campaign accounting. It does not claim full parity: upstream native/client/benchmark declarations, generated cases, configurations, platforms and external suites remain unexecuted.",
