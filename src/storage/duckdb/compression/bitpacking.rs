...
macro_rules! decoder {
    ($name:ident, $label:literal, $unpack:ident) => {
        ...
        impl SegmentDecoder for $name {
            ...
            fn supports(&self, kind: SegmentType<'_>) -> bool { matches!(kind, SegmentType::Values(t) if t.is_integer() || t.is_decimal() || (t.is_temporal() && *t != DataType::Interval) || matches!(t, DataType::Date | DataType::Uuid | DataType::Enum(_))) }
