...
impl SegmentDecoder for RleDecoder {
    ...
    fn supports(&self, kind: SegmentType<'_>) -> bool {
        matches!(kind, SegmentType::Values(t) if t.is_integer() || t.is_decimal() || t.is_floating() || t.is_temporal() || matches!(t, DataType::Boolean | DataType::Date | DataType::Uuid | DataType::Enum(_)))
    }
