...
pub(super) fn expression(
    ...
    let kind = match (class, kind) {
        ...
        (4, 203) => column_reference(reader, state)?,
...
#[cfg_attr(feature = "dev", duckdb_dev::instrument)]
fn column_reference(reader: &mut Reader, state: &mut State<'_>) -> Result<StoredExpressionKind> {
    if !reader.optional(200)? {
    ...
    if count == 1 && parts[0].eq_ignore_ascii_case("current_timestamp") {
        Ok(StoredExpressionKind::CurrentTimestamp)
    } else {
        Ok(StoredExpressionKind::ColumnReference(parts))
    }
}
