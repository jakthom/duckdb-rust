...
fn feedback(
    ...
    if !hex_hash(expected_token_sha256) || sha256(token_path)? != expected_token_sha256 {
        return Err("feedback token changed".into());
    }
    ...
    if token.kind != "duckdb-rust-selected-feedback"
        || token.version != 1
        || !matches!(token.target.as_str(), "release" | "development")
        || token.revision.len() != 40
        || token.workload_id.is_empty()
        || token.sample_id.is_empty()
        || token.timeout_ms == 0
        || !hex_hash(&token.source_sha256)
        || !hex_hash(&token.suite_sha256)
        || !hex_hash(&token.runner_binary_sha256)
        || !hex_hash(&token.runner_source_sha256)
        || !hex_hash(&token.provenance_sha256)
        || token.runner_profile != "release"
        || std::path::Path::new(&token.path).is_absolute()
        || token.path.contains("..")
    {
    ...
    if !path.starts_with(&root)
        || std::fs::metadata(&path).map_err(|e| e.to_string())?.len() != token.source_bytes
        || sha256(&path)? != token.source_sha256
    {
        return Err("feedback source bytes changed".into());
    }
    let executable = std::env::current_exe()
        .map_err(|error| format!("current runner: {error}"))?
        .canonicalize()
    ...
    if executable
        != token
            .runner_path
            .canonicalize()
            .map_err(|error| format!("token runner: {error}"))?
        || sha256(&executable)? != token.runner_binary_sha256
    {
        return Err("feedback runner identity changed".into());
    }
    ...
    if !declared {
        return Err("feedback suite metadata omits selected source bytes".into());
    }
    let result = duckdb_rust::Database::memory().and_then(|db| runner::run_file_report(&db, &path));
    let value = match result {
        Ok(file)
            if matches!(file.status, runner::FileStatus::Passed)
                && file.passed > 0
                && file.skipped == 0
                && file.generated == 0 =>
        {
    ...
    if sha256(token_path)? != expected_token_sha256
        || sha256(&path)? != token.source_sha256
        || sha256(&token.suite_path)? != token.suite_sha256
    {
    ...
    report
        .write_all(&serde_json::to_vec(&value).expect("JSON report"))
        .map_err(|error| format!("write feedback report: {error}"))?;
    if value["status"] != "passed" {
        return Err("feedback SQLLogic assertion failed".into());
    }
...
fn main() -> std::process::ExitCode {
    ...
    if arguments
        .first()
        .is_some_and(|argument| argument == "--feedback-token")
    {
        if arguments.len() != 6
            || arguments[2] != "--token-sha256"
            || arguments[4] != "--feedback-report"
        {
            eprintln!(
                "Usage: sqllogictest --feedback-token TOKEN --token-sha256 SHA256 --feedback-report REPORT"
            );
