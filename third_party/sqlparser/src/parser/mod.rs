...
impl<'a> Parser<'a> {
    ...
    pub fn parse_enum_values(&mut self) -> Result<Vec<EnumMember>, ParserError> {
        self.expect_token(&Token::LParen)?;
        if dialect_of!(self is DuckDbDialect) && self.consume_token(&Token::RParen) {
            // DuckDB accepts the syntax and rejects empty declarations at binding.
            return Ok(Vec::new());
        }
    ...
    fn parse_data_type_helper(
        ...
        let mut data = match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                Keyword::MAP if dialect_is!(dialect is DuckDbDialect | ClickHouseDialect | GenericDialect) => {
                    self.prev_token();
                ...
                Keyword::TUPLE if dialect_is!(dialect is DuckDbDialect | ClickHouseDialect | GenericDialect) => {
                    self.prev_token();
